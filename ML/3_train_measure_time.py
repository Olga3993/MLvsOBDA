...
from datasets_predict.init_dataset import init_dataset
from datasets_predict.dataset import Dataset
...
def get_feats_torch(feats, feat_type, dataset_name):
    # if feat_type == 'dummy' and dataset_name != 'polblogs':
...
def get_masks(n,
              ...
              val_ratio,
              seed=1):
    train_mask, val_mask = np.zeros(n), np.zeros(n)
    x_train, x_val, y_train, y_val = train_test_split(main_ids,
                                                    main_labels,
                                                    stratify=main_labels,
                                                    test_size=val_ratio,
                                                    random_state=seed)
    train_mask[x_train], val_mask[x_val] = 1, 1
...
def train_predict_node_clf(dataset: Dataset,
                           masked_fname,
                           feature_params,
                           val_ratio=0.1,
                           seed=1,
    ...
    save_fname = f'{model_name}___{masked_fname.split(".")[0]}'

    dataset.set_feat_params(feature_params)
    masked_data, full_data = dataset.get_data(masked_fname, seed)
    masked_data['features'] = get_feats_torch(masked_data['features'], dataset.feature_params['feat_type'], dataset.dataset_name).to(device)
    ...
    train_mask, val_mask = get_masks(n_nodes_in_masked,
    ...
    for epoch in range(n_epochs):
        ...
        f1, _ = evaluate(model, g, masked_data['features'], masked_data['labels'], val_mask)
        scheduler.step(1 - f1)
        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), dataset.dataset_folder / 'models' / f'{save_fname}.pt')
    ...
    model.load_state_dict(torch.load(dataset.dataset_folder / 'models' / f'{save_fname}.pt'))
    f1_train, _ = evaluate(model, g, masked_data['features'], masked_data['labels'], train_mask)
    f1_val, _ = evaluate(model, g, masked_data['features'], masked_data['labels'], val_mask)
    print("Train F1 {:.3}".format(f1_train), "Val F1 {:.3}".format(f1_val))
    ...
    full_data['features'] = get_feats_torch(full_data['features'], dataset.feature_params['feat_type'], dataset.dataset_name).to(device)
    full_data['labels'] = torch.LongTensor(full_data['labels']).to(device)
    # print(full_data['features'].shape, masked_data['features'].shape)
    start = time.time()
    f1_test_full, predicted = evaluate(model, g_full, full_data['features'], full_data['labels'], full_data['eval_mask'])
    end = time.time()
    print('Predict:', end-start)
    # if verbose:
    # print()
    # print("Train F1 {:.3}".format(f1_train), "Val F1 {:.3}".format(f1_val),
    #       "Test F1 Full {:.3}".format(f1_test_full))
    # print('---------------')
...
def main():
    ...
    dataset_to_models_params = read_json('config/best_params3.json')['model_params']
    # dataset_to_models_params = {'polblogs': dataset_to_models_params['polblogs']}
    for dataset_name in (dataset_to_models_params):
        ...
        for model_params in dataset_to_models_params[dataset_name]:
            ...
            masks_fnames = ['10_50_1.npy', '50_50_1.npy', '90_50_1.npy'] # os.listdir(dataset.dataset_folder / 'masks_new')
            for i, masked_fname in tqdm(list(enumerate(masks_fnames))):
                # print(f'{i}/{len(masks_fnames)}')
                print(masked_fname)
                train_predict_node_clf(dataset,
                                       masked_fname=masked_fname,
